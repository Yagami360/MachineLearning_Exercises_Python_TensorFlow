...
### 実行条件１
- ｛隠れ層：**128** ノード、多層 RNN の層数: **1** ｝の LSTM
    - 各シーケンス長 : 250 個
    - 学習率 0.001, 最適化アルゴリズム : Adam ( 減衰項 : beta1 = 0.9, beta2 = 0.999 )
    - トレーニング用データ : 50 %、テスト用データ : 50% に分割 
    - エポック回数 **20**, ミニバッチサイズ 100、イテレーション回数 25000


##### 損失関数のグラフ（実行条件１）
![rnn-lstm_3-1-2](https://user-images.githubusercontent.com/25688193/39128789-aa38365e-4743-11e8-817a-51311a380245.png)

<!-->
> ミニバッチのイテレーション回数が xxxx 回程度で 0 に収束しており、うまく学習出来ていることが見て取れる。
-->
...
|---|---|---|
|total||50,000|
|train data|0.xxx|25,000 (50.0%)|
|test data|0.xxx|25,000 (50.0%)|
...
### 予想出力値と元データの波形図（時系列データ）: `main2.py`
